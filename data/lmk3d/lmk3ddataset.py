...
def random_occlusion(image):
    if random.random() > 0.5:
        ...
        image_np[occ_ymin:occ_ymin + occ_height, occ_xmin:occ_xmin + occ_width,
                 0] = int(random.random() * 255)
        image_np[occ_ymin:occ_ymin + occ_height, occ_xmin:occ_xmin + occ_width,
                 1] = int(random.random() * 255)
        image_np[occ_ymin:occ_ymin + occ_height, occ_xmin:occ_xmin + occ_width,
                 2] = int(random.random() * 255)
        image_pil = Image.fromarray(image_np[:, :, ::-1].astype('uint8'),
                                    'RGB')
        return image_pil
    else:
        return image
...
def random_rotate(image, target, angle_max):
    if random.random() > 0.5:
        ...
        target_center = np.array(target) - np.array(
            [center_x, center_y] * landmark_num)
        ...
        target_rot = target_center_rot.reshape(landmark_num * 2) + np.array(
            [center_x, center_y] * landmark_num)
...
def gen_target_pip(target, meanface_indices, target_map, target_local_x,
                   target_local_y, target_nb_x, target_nb_y):
...
class LMK3DDataSet(Dataset):
    ...
    def __init__(self,
        ...
        self.image_lst = glob.glob(
            '/home/xrsz/Pictures/landmark_data_base/landmark_dataset/*/images/*'
        )
    ...
    def __getitem__(self, index):
        ...
        target_map = np.zeros(
            (self.num_lms, int(self.input_size / self.net_stride),
        ...
        target_local_y = np.zeros(
            (self.num_lms, int(self.input_size / self.net_stride),
             int(self.input_size / self.net_stride)))
        target_nb_x = np.zeros((self.num_nb * self.num_lms,
                                int(self.input_size / self.net_stride),
                                int(self.input_size / self.net_stride)))
        target_nb_y = np.zeros((self.num_nb * self.num_lms,
                                int(self.input_size / self.net_stride),
                                int(self.input_size / self.net_stride)))
        target_map, target_local_x, target_local_y, target_nb_x, target_nb_y = gen_target_pip(
            target, self.meanface_indices, target_map, target_local_x,
            target_local_y, target_nb_x, target_nb_y)
...
if __name__ == "__main__":
    ...
    for idx, (img, lmk, angles) in enumerate(dataset):
        lmks.append(np.array(lmk).transpose(1, 0))

    meanface = np.array(lmks)
    meanface = (np.mean(meanface, axis=0).reshape(-1, 3) * 224).astype(
        np.int32)

    img = np.zeros([224, 224, 3])
    for pt in meanface[:, :2]:
